%define xulver $(rpm -q --qf=%%{VERSION} xulrunner)
...
%patch22 -p1 -b .fix22
%ifarch %{arm}
# just use the same Linux.mk for all kernel versions.
# Koji builds on many different kernels
sed -i -e 's/OpenVMS/OpenVMS Linux/' esc/coreconf/config.mk
%endif

%build

export GECKO_VER=%{xulver}
...
%ifarch %{arm}
export LDFLAGS="-lm -lnss3 -lplc4 -lplds4 -lX11"
%endif
...
%changelog
* Sat Sep 17 2016 Bjarne Saltbaek <bjarne@redsleeve.org> 1.1.0-26.0
- modified to build on any xulrunner
- modified to build on any Linux kernel using the default config

* Thu Nov 21 2013 Johnny Hughes <johnny@centos.org> 1.1.0-26.el6.centos
- modified to build on xulrunner 17.0.10 instead of 17.0.8
