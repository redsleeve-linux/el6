...
# fix for --listed-incremental segfault when backing up the root directory
# for the second and subsequent invocation (#653433)
Patch8: tar-1.23-listedincremental.patch
# fix for filename corruption when --sparse and --posix options are used.
# (#656834)
Patch9: tar-1.23-long-name-corruption.patch
# fix for data lost bug when --remove-files option used.
Patch10: tar-1.23-remove-files.patch
# endless loop fix when symlink was extracted with -k option.
Patch11: tar-1.23-keep-old-files-endless-loop.patch
# optimization for -c --sparse for completely sparse files  (#760665)
Patch12: tar-1.23-optimize-packing-entirely-sparse-files.patch
# use gnulib's fnmatch rather than glibc's
Patch13: tar-1.23-gnulib-fnmatch.patch
# --remove-files in combination with --append did not remove archived files,
# this resolves #782628
Patch14: tar-1.23-remove-files-update-append.patch
# --update and -C didn't work correctly together (#688567)
Patch15: tar-1.23-update-with-change-directory.patch
# remove coverity detected UNINIT defect added by Patch11
Patch16: tar-1.23-uninit-in-extract-node.patch
# switch back the traditional --keep-old-files behaviour and add new option
# --skip-old-files
Patch17: tar-1.23-add-skip-old-files-option.patch
# disable tests/remfiles01.at for running as a privileged user (#687972)
Patch18: tar-1.23-testsuite-root.patch
# Fix undesired error exit on receiving SIGPIPE during listing (#807728)
Patch19: tar-1.23-silence-sigpipe-warn-when-listing.patch
# Add AT_SIGPIPE_PREREQ into testsuite
Patch20: tar-1.23-sigpipe-prereq-testsuite.patch
...
%patch8 -p1 -b .listedincremental
%patch9 -p1 -b .long-name-corruption
%patch10 -p1 -b .remove-files
%patch11 -p1 -b .keep-old-files-dead-loop
%patch12 -p1 -b .completely-sparse-files
%patch13 -p1 -b .use-gnulib-fnmatch
%patch14 -p1 -b .remove-files-and-append
%patch15 -p1 -b .update-and-chage-dir
%patch16 -p1 -b .uninit-in-extract-node
%patch17 -p1 -b .skip-old-files-option
%patch18 -p1 -b .root-testsuite-fix
...
* Mon Sep 24 2012 Pavel Raiskup <praiskup@redhat.com> 2:1.23-8
- fix for bad file matching during extracting concrete files from an archive
  (backported fix from fedora 14) (#841308)
- typo - better separate comments in spec file

* Thu Mar 06 2012 Pavel Raiskup <praiskup@redhat.com> 2:1.23-7
- disable tests/remfiles01.at for running as a privileged user (#687972)
...
* Fri Mar 05 2012 Pavel Raiskup <praiskup@redhat.com> 2:1.23-6
- fix for uninit defect (revealed by Coverity) added with %patch11.
- recovered traditional behaviour of '--keep-old-files' option and added new
  option '--skip-old-files' (#799252)

* Thu Mar 01 2012 Pavel Raiskup <praiskup@redhat.com> 2:1.23-5
- fix for bad cooperation of --update (-u) and --directory (-C) options.
  (#688567)

* Thu Feb 16 2012 Pavel Raiskup <praiskup@redhat.com> 2:1.23-4
- fix crash when the --listed-incremental option is used and archive includes
  root directory (#653433)
- fix for long name corruption when --posix and --sparse options are used,
  testcase included (#656834)
- using --remove-files caused data lost when symlinks were contained.  Testcase
  is included (#698212)
- endless loop fix, problem occurred when the `-k' option was used and extracted
  archive contained symlink that already existed in file system (#726723).
- improve sparse file support performance for entirely sparse files (#760665).
- use gnulib's fnmatch rather then glibc's (in RHEL-6) (#768724)
- the --remove-files option did not remove archived files in combination with
  append option (-r) (#782628).
